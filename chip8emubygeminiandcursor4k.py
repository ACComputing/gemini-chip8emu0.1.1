...
class Chip8EmulatorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Geminis's chip8 emulator 0.1")
        ...
        self.root.title("Geminis's chip8 emulator 0.1 - No ROM Loaded")
    ...
    def open_rom(self):
        ...
        if filepath:
            try:
                ...
                self.paused = False
                self.btn_pause.config(text="PAUSE")
                self.root.title(f"Geminis's chip8 emulator 0.1 - {filepath.split('/')[-1]}")
            except Exception as e:
                print(f"Error loading ROM: {e}")

    def emulation_loop(self):
        """Runs in a background thread to maintain CPU speed independent of Tkinter"""
        cycles_per_frame = 10 # Adjust for speed (CHIP-8 typically runs 500-700Hz, 10 ops * 60fps = 600Hz)
    ...
    def update_screen(self):
        if self.cpu.draw_flag:
            ...
            for y in range(32):
                for x in range(64):
                    idx = x + (y * 64)
                    color = "#00FF00" if self.cpu.gfx[idx] else "black" # Matrix green pixels!
